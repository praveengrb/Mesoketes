...
public class AttackFromNorthSide<T> extends AttackFrom<T> {
	...
	public WarResult<T> attack(WarResult<T> warResult) {
		...
		if (tribe.getStrength() > warResult.getCompoundWall().getNorth().getHeight()) {
			...
			warResult.getCompoundWall().getNorth().setHeight(tribe.getStrength());
